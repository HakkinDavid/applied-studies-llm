...
def get_knowledge_forest():
    ...
    return KnowledgeForestResponse(total_trees=total_trees, forest=forest)
...
def clear_knowledge_forest():
    ...
    return {"status": "ok", "message": "Bosque de conocimiento borrado.", "cleanup": cleanup}


@router.post("/api/cleanup/orphans")
def run_orphan_cleanup():
    cleanup = cleanup_orphans()
    return {"status": "ok", "message": "Limpieza de huérfanos ejecutada.", "cleanup": cleanup}
